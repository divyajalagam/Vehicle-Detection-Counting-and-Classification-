...
cap=cv2.VideoCapture("C:\\Users\\jsaid\\OneDrive\\Desktop\\vehical\\Videos\\{filename}") 
...
while(cap.isOpened()):
    ret,frame=cap.read() 
    frame=cv2.resize(frame,(600,300))#(900,500)
    for i in cars:
        i.age_one()
...
#------------------------------------------------------BINARIZATION----------------------------------------------------------------------------
    if ret==True:
                    ...
                    for i in cars:
                        ...
                        if i.getState()=='1':
                            if i.getDir()=='down'and i.getY()>down_limit:
                                i.setDone()
                            if i.getDir()=='up'and i.getY()<up_limit:
                                i.setDone()
                        if i.timedOut():
                            index=cars.index(i)
                            cars.pop(index)
        ...
        str_up='Up_Count: '+str(cnt_down)
        str_down='Down_count: '+str(cnt_down)
        ...
        cv2.putText(frame, str_down, (10, 60), font, 0.5, (255, 0, 0), 2, cv2.LINE_AA)
...
def home():
    if request.method == 'POST':
        ...
        if user == 'admin' and password == 'admin':
            return redirect(url_for('vehicles_detection'))
        else:
            return render_template('login.html')
    
    return render_template('login.html')

@app.route('/vehicles_detection',methods=['POST','GET'])
